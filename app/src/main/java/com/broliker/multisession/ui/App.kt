...
private fun BrowserScreen(
    ...
    fun configureProfile(webView: WebView) {
        if (multiProfileSupported) {
            runCatching {
                // The profile must be bound before configuring/using the WebView.
        ...
        webView.settings.apply {
            // Match the working app's classic WebView approach rather than
            // forcing a stale Chrome UA.
            ...
            // Intentionally do not override userAgentString.
        }
    ...
    fun makeChromeClient(webView: WebView): WebChromeClient = object : WebChromeClient() {
        ...
        override fun onCreateWindow(
            ...
            // Keep only the popup visible while it is active.
        ...
        Box(Modifier.fillMaxSize().padding(pad)) {
            AndroidView(
                ...
                factory = { ctx ->
                    FrameLayout(ctx).also { root ->
                        ...
                        // Do this only after profile + settings + clients are ready.
            ...
            errorMsg?.let { err ->
                Column(
                    ...
                    Text("Page failed to load", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                    Spacer(Modifier.height(8.dp))
                    Text(err, style = MaterialTheme.typography.bodySmall)
                    Spacer(Modifier.height(8.dp))
                    Text(currentUrl, style = MaterialTheme.typography.labelSmall)
                    Spacer(Modifier.height(16.dp))
                    Button(onClick = { errorMsg = null; activeWebView?.reload() }) {
                        Text("Reload")
                    }
                }
            }
